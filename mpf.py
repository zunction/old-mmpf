...
def sgd(units = 16, learning_rate = 0.01, n_epochs = 1000, batch_size = 16,  timestr = None):
    """
    Perform stochastic gradient descent on MPF
    """
    print ('Loading '+ 'sample' + timestr + '.dat.npy' + '...')

    dataset = load_data('sample' + timestr + '.dat.npy')
...
#     os.chdir(output_folder)
    ...
    W = np.load('W' + timestr + '.dat.npy')
    b = np.load('b' + timestr + '.dat.npy')
    ...
    savefilename = timestr + 'result.png'
...
if __name__ == "__main__":
    W_learnt, b_learnt = sgd(units = 32, learning_rate = 0.01, n_epochs = 1000, \
    batch_size = 16, timestr = '32')
